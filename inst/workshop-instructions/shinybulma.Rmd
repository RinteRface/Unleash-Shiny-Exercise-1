...
1. The CDN url is **https://cdn.jsdelivr.net/npm/bulma@0.9.0**. In code chunk below, replace `...` by anything appropriate.
...
**Note**: at run time, shiny adds `html` around the UI, thereby making is not necessary to include. 

2. In the ./R folder, create the `bulma_page` function that has 2 parameters: `...` and `title` (default to NULL). The function body is composed of the template head and the template body. Hint: wrap `head` and `body` in a `tagList`.
...
7. Buttons may contain icons. Using `tagAppendChild` and `shiny::icon`, add an icon tag to the `bulma_button` function.
